  ...
  return (
    <div className='border p-6 shadow rounded'>
      <div className='flex justify-between items-center'>
        <img className='h-8' src={assets.profile_img} alt="" />
      </div>
      ...
      <p className='text-gray-500 text-sm mt-4' dangerouslySetInnerHTML={{ __html: device.description ? device.description.slice(0, 150) : '' }}></p>
      <div className='mt-8 mb-1 flex gap-4 text-sm '>
        <button
          onClick={() => {
            console.log("Navigating to apply-device with ID:", device._id);
            navigate(`/apply-device/${device._id}`);
            window.scrollTo(0, 0);
          }}
